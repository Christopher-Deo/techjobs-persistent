...
public class TestTaskFour extends AbstractTest {
    ...
    * */
    ...
    public void testSqlQuery() throws IOException {
        ...
        Pattern queryPattern = Pattern.compile("SELECT\\s+DISTINCT\\s+s.name\\s+FROM\\s+skill\\s+s\\s+" +
                "JOIN\\s+job_skills\\s+js\\s+ON\\s+s.id\\s+=\\s+js.skills_id\\s+" +
                "ORDER\\s+BY\\s+s.name\\s+ASC;", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
